...
rollBtn?.addEventListener("click", async () => {
  ...
  /* -----------------------------------------------
     ASK SERVER FOR AUTHORITATIVE RESULT
  ----------------------------------------------- */
  ...
  try {
    ...
    if (
      ...
      !data.code ||
      !Array.isArray(data.colours) ||
      data.colours.length !== diceCount
    ) {
      throw new Error(
        "Invalid roll response from server."
      );
    }
    ...
    /* ---------------------------------------------
       ...
       We display exactly what was saved.
    --------------------------------------------- */
    ...
    /* Reveal the final dice */

    for (let i = 0; i < data.colours.length; i++) {

      const partial = data.colours
        .slice(0, i + 1);

      const placeholders = data.colours
        .slice(i + 1)
        .map(() => secureColour());
    ...
    if (rollStatus) {
      rollStatus.textContent =
        "✓ Roll complete and saved as a verified result.";
      rollStatus.classList.add("success");

      setTimeout(() => {
        rollStatus.classList.remove("success");
      }, 2500);
    }

    const sessionRolls =
      document.querySelector("#sessionRolls");
    ...
    if (rollStatus) {
      rollStatus.textContent =
        "⚠️ The roll could not be verified. Please try again.";
      rollStatus.classList.add("error");
